...
function initialize() {
    ...
    viz_footer = viz.selection()
        .select("svg")
        .append("text")
        .attr("class", "title")
        .attr("x", 100)
        .attr("y", 575).attr("text-anchor", "middle")
        // .style("fill", "#FFF")
        .style("font-family", "Raleway")
        .style("font-weight", 300);
...
function onUpdate() {
    ...
    viz_title.transition().text("Sentiment Impact");
    viz_footer.transition().text("Bad Sentiment")
    viz_footer_two.transition().text("Good Sentiment")
    viz_title.transition().attr("x",125);
}
...
var datatip = '<div class="tooltip" style="width: 220px; background-opacity:.5">' +
    ...
    '<div class="header1">Sentiment Score: HEADER1</div>' +
    '<div class="header-rule"></div>' +
    '<div class="header3" style="font-size:12px;"> Impact Score: HEADER3 </div>' +
    '</div>';
