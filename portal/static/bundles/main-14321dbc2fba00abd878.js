...
/***/ function(module, exports, __webpack_require__) {
	...
	var BooksList = React.createClass({
	    ...
	    render: function () {
	        if (this.state.data) {
	            var texts = this.state.data.map(function (x, i) {
	                return x.texts[i];
	                // return <td>{x.texts[i].id}}</td><td>{ x.texts[i].date_created }</td><td>{ x.texts[i].phone_number }</td><td>{ x.texts[i].user_id }</td><td>{ x.texts[i].message }</td><td>{ x.texts[i].analysis }</td><td>{ x.texts[i].resolution }</td>
	            });
	        }
	...
	ReactDOM.render(React.createElement(BooksList, { pollInterval: 1000 }), document.getElementById('container'));
