...
function formBuilderHelpersFn(opts, formBuilder) {
  ...
  _helpers.getData = function () {
    ...
    // $.getScript("/static/js/user/dashboard.js")
       ...
       rel: 'stylesheet',
       type: 'text/css',
       href: "/static/assets/js/plugins/formBuilder/dist/form2.css"
    }).appendTo('head');
  ...
  _helpers.updatePreview = function (field) {
    ...
    if (previewData.type == 'checkbox-group') {
     $.getScript("/static//js/user/dashboard_chart.js")    
    }
  ...
  _helpers.fieldPreview = function (attrs) {
    ...
    switch (attrs.type) {
      ...
      case 'submit':
        ...
        // $.getScript("/static/js/user/dashboard.js")
        $("#portsummary").css('display','initial')
        var a = $("#portsummary")[0].outerHTML
        $("#portsummary").css('display','none')
        preview = a+'<button style="display:none " ' + attrsString + '>' + attrs.label + '</button>';
        break;
      case 'select':
        var options = '',
      ...
      case 'hidden':
      case 'checkbox':
        var clone = $("#toppicks").clone().css('display','initial')
        preview = clone[0].outerHTML+'<input type="' + attrs.type + '" ' + toggle + ' style="display:none"> ';
        // x.css('display','none !important')
        break;
      case 'autocomplete':
        preview = '<div class="block" style="height:599px;margin:0px" id="ha"><div class="header"><input class="ui-autocomplete-input ' + attrs.className + '" autocomplete="on" style="display:none"><h2 style="font-size:12px;font-weight:300;-webkit-font-smoothing: antialiased;font-family:"helvetica neue"">Your Portfolios</h2></div>';
        break;
      default:
        attrsString = utils.attrString(attrs);
  ...
  var FormBuilder = function FormBuilder(options, element) {
    ...
    var defaults = {
      ...
      messages: {
        ...
        autocomplete: 'Alerts',
        button: 'Portfolio Summary',
        cannotBeEmpty: 'This field cannot be empty',
        checkboxGroup: 'Market Indexes',
        checkbox: 'Top Picks',
        ...
        fileUpload: 'DOW JONES Index',
        formUpdated: 'Form Updated',
        getStarted: 'Drag a field from the right to this area',
        header: 'NY Times Headlines',
        hide: 'Edit',
        hidden: 'asdf',
        label: 'Label',
        ...
        other: 'Other',
        paragraph: 'The Guardian Headlines',
    ...
    if (opts.showActionButtons) {
      ...
      var viewDataText = opts.dataType === 'xml' ? opts.messages.viewXML : opts.messages.viewJSON,
        ...
        id: frmbID + '-view-data',
        type: 'button',
        className: 'view-data btn btn-default hidden'
      }),
    ...
    var advFields = function advFields(values) {
      ...
      if (opts.typeUserAttrs[values.type]) {z
    ...
    var appendNewField = function appendNewField(values) {
      ...
      var liContents = utils.markup('div', [], { className: 'field-actions' }).outerHTML;

      // // Field preview Label
      // liContents += '<label class="field-label" style="height:0p !important"> </label>';
